...
var _ = Describe("ClusterStackReconciler", func() {
	Context("autosubscribe false", func() {
		var (
		...
		It("makes sure release assets are downloaded properly", func() {
			By("checking that it creates the cluster stack release object")
			Eventually(func() error {
				var clusterStackRelease csov1alpha1.ClusterStackRelease
				return testEnv.Get(ctx, clusterStackReleaseKey, &clusterStackRelease)
			}, timeout, interval).Should(BeNil())

			By("checking if the AssetsClientAPIAvailableCondition condition is true")
	...
	Context("autosubscribe true", func() {
		var (
			clusterStack           *csov1alpha1.ClusterStack
			testNs                 *corev1.Namespace
